...
class PdfTocExtraction(PdfParserState):
    ...
    def _find_toc_titles(self) -> list[TocTitle] | None:
        ...
        # browse through lines up to page 10 to get those which might be TOC
        for i, line in enumerate(filter(lambda x: x.page<10, self.lines)):
            until_last_match_counter += 1
            if until_last_match_counter > 5 and len(toc_titles) > 3:
