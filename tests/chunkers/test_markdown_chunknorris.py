...
def test_check_string_argument_is_valid():
    try:
        ...
        MarkdownChunkNorris._check_string_argument_is_valid(
            "header_style", "invalid_value"
        )
    except AssertionError as e:
        assert (
            str(e)
            == "Argument 'header_style' should be one of ['setext', 'atx']. Got 'invalid_value'"
        )
...
def test__change_links_format(md_with_links, output_change_links_format):
    for arg in ["in_sentence", "end_of_chunk", "remove", "leave_as_markdown"]:
        assert (
            MarkdownChunkNorris.change_links_format(md_with_links, arg)
            == output_change_links_format[arg]
        )
...
def test_split_big_chunks(mdcn):
    ...
    assert (
        len(
            mdcn.split_big_chunks(
                chunks, max_chunk_tokens=3, chunk_tokens_exceeded_handling="split"
            )
        )
        == 3
    )
    assert (
        len(
            mdcn.split_big_chunks(
                chunks, max_chunk_tokens=6, chunk_tokens_exceeded_handling="split"
            )
        )
        == 2
    )
    try:
        mdcn.split_big_chunks(
            chunks, max_chunk_tokens=3, chunk_tokens_exceeded_handling="raise_error"
        )
